...
- `_includes`： 构成本网站的`html`代码，不建议修改
- `_layouts`： 构成本网站的`html`代码，不建议修改
- `assets`： 美化本网站的`css,less,js`代码，不建议修改
- `backup`： 用于备份文件，以便于后续修改时可以参考
- `blogs`： 存放个人博客`.md`文件，以及对应的图片素材
- `file`： 存放简历CV等个人文件，用于设置访问链接
- `images`： 存放`.jpg .png`等媒体文件，用于设置访问链接
- `mypaper`： 显而易见，存放个人学术文章，用于设置访问链接
